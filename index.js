  ...
  if (interaction.isButton()) {
    if (interaction.customId == "buttonVerify") {
      ...
      let username_ic = new TextInputBuilder()
        ...
        .setLabel("อายุ OC".substring(0, 45))
        .setStyle(TextInputStyle.Short);

      let age_oc = new TextInputBuilder()
        .setCustomId("age_oc")
        .setLabel("รู้จักประเทศนี้ได้จากทีไ่หน".substring(0, 45))
        .setStyle(TextInputStyle.Paragraph);

      let facebook_ic = new TextInputBuilder()
        .setCustomId("facebook_ic")
        .setLabel("ลิ้งค์เฟส IC หรือ OC".substring(0, 45))
        .setStyle(TextInputStyle.Short)
        .setRequired(true);
    ...
    if (interaction.customId == "addRoles") {
      ...
      try {
      await m.send("คุณได้รับ **WHITELSIT** ของ Ocean Town แล้ว");
      } catch (error) {
    ...
    if (interaction.customId == "Cancel") {
      ...
      try {
        await m.send("คุณกรอกข้อมูลไม่ครบถ้วน กรุณากรอกใหม่ที่ <#1168194228917121064>");
        } catch (error) {
        console.error("เกิดข้อผิดพลาดในการส่งข้อความ: ", error);
        }
    }
  ...
  if (interaction.isModalSubmit()) {
    try {
      ...
      let x = {
        ...
        "ชื่อ IC": username_oc,
        "อายุ OC": username_ic,
        "รู้จักประเทศนี้ได้จากทีไ่หน": age_all,
        "เฟส IC": facebook_ic,
      };
      data.push(x);
      file.set("data", data);
